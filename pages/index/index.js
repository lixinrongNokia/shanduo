...
Page({
  ...
  onLoad: function () {
    ...
    WxNotificationCenter.addNotification('userLoginNotification', that.userLoginNotification, that);
    WxNotificationCenter.addNotification('authReqNotification', that.authReqNotification, that);
    that.setData({ host: app.host, isLogin: app.globalData.userInfo ? true : false, isauthReq: app.globalData.isauthReq })
    wx.getSystemInfo({
  ...
  //用户登录状态监听
  userLoginNotification: function (isLogin) {
    console.log('用户登录了');
    this.setData({ isLogin: isLogin })
  },
  authReqNotification: function (obj) {
    console.log('请求授权');
    this.setData({ isauthReq: obj });
  },
  onUnload: function () {
  ...
  }, gotoCreditCenterView: function (e) {
    var userId = e.currentTarget.dataset.current;
    if (app.globalData.userInfo) {
      wx.navigateTo({
        url: '/pages/credit/credit?userId=' + userId + ''
      });
    } else {
      util.toast('登录后查看');
    }
