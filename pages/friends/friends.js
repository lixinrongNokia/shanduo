...
Page({
  ...
  loadFriends: function () {
    ...
    wx.request({
      ...
      data: {
        token: token, typeId: 1, page: that.data.pageIndex, pageSize: that.data.pageCount
      },
      dataType: 'json',
      header: {
      ...
      success: (res) => {
        if (res.data.success) {
          ...
          var oldLetters = that.data.letters;
          that.data.groups = oldGroup.concat(groups);
          that.data.letters = oldLetters.concat(letters);
          that.setData({ groups: that.data.groups, letters: that.data.letters });
        }
      }
    })
  },
